...
int main(int argc, char* argv[]){
	...
	if(argc == 6){
		...
		LINE_START  = std::atoi(argv[4]) - 1;
		LINE_END    = std::atoi(argv[5]) - 1;
	}
	else if(argc == 5){
		PATHS_PATH = argv[1];
		COLUMN_TRUE = std::atoi(argv[2]) - 1;
		...
		std::cerr << "Usage: ./error PATHs.txt COLUMN_TRUE COLUMN_DATA LINE_START [LINE_END](1 start, comment is not counted)" << std::endl;
		return 1;
	}
